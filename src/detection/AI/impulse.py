...
def build_model(input_shape: tuple, weights: str, alpha: float,
    ...
    mobile_net_v2 = MobileNetV3Small(input_shape=input_shape,
                                weights=weights,
                                alpha=alpha,
                                include_top=True)
...
def train(num_classes: int, learning_rate: float, num_epochs: int,
    ...
    if input_num_channels == 1:
        if alpha == 0.1:
            weights = os.path.join(WEIGHTS_PREFIX, "transfer-learning-weights/edgeimpulse/MobileNetV2.0_1.96x96.grayscale.bsize_64.lr_0_05.epoch_441.val_loss_4.13.val_accuracy_0.2.hdf5")
        elif alpha == 0.35:
            weights = os.path.join(WEIGHTS_PREFIX, "transfer-learning-weights/edgeimpulse/MobileNetV2.0_35.96x96.grayscale.bsize_64.lr_0_005.epoch_260.val_loss_3.10.val_accuracy_0.35.hdf5")
    elif input_num_channels == 3:
        if alpha == 0.1:
            weights = os.path.join(WEIGHTS_PREFIX, "transfer-learning-weights/edgeimpulse/MobileNetV2.0_1.96x96.color.bsize_64.lr_0_05.epoch_498.val_loss_3.85.hdf5")
        elif alpha == 0.35:
            weights = os.path.join(WEIGHTS_PREFIX, "transfer-learning-weights/keras/mobilenet_v2_weights_tf_dim_ordering_tf_kernels_0.35_96.h5")
