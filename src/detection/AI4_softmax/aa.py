...
def build_model(
    input_shape: tuple, weights: str, alpha: float, num_classes: int
) -> tf.keras.Model:
    ...
    mobile_net_v2 = MobileNetV2(
        input_shape=input_shape, weights=weights, alpha=alpha, include_top=True
    )
    ...
    model = Conv2D(
        filters=32, kernel_size=1, strides=1, activation="relu", name="head"
    )(cut_point.output)
    logits = Conv2D(
        filters=num_classes, kernel_size=1, strides=1, activation=None, name="logits"
    )(model)
...
def train(
    ...
    validation_dataset_for_callback = validation_dataset.batch(
    ...
    if lr_finder:
        learning_rate = ei_tensorflow.lr_finder.find_lr(
            model, train_segmentation_dataset, weighted_xent
        )
