...
if __name__ == "__main__":
    ...
    raw_data_dir = "../rawdata2"
    img_path_list = glob.glob(os.path.join(raw_data_dir, "*.jpg"))
    class_names = [c.strip() for c in open("./data/coco.names").readlines()]
    # print(88888888888)
    for img_path in img_path_list:
        img = tf.image.decode_image(
            open(os.path.join(raw_data_dir, img_path), "rb").read(),
            channels=3,
        )
        ...
        img = draw_outputs(img, (boxes, scores, classes, nums), class_names)
        cv2.imwrite(os.path.join(raw_data_dir, "train/images", img_path), img)
        print(img_path)
